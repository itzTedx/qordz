...
export const OverviewTabs = () => {
  return (
    <div className="mt-12">
      <nav className="sticky top-4 z-50 mx-auto mb-4 flex w-fit gap-1 overflow-hidden rounded-full bg-muted p-1">
        {TABS.map((item) => (
          <Link className="block rounded-full bg-card px-4 py-2 shadow-sm" href={`#${item.href}`} key={item.href}>
            {item.label}
          </Link>
        ))}
      </nav>
