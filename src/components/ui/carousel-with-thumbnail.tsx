...
const getAspectRatioClass = (ratio?: string) => {
  switch (ratio) {
    ...
    case "wide":
      return "aspect-4/3 sm:aspect-5/3 lg:aspect-6/4"; // 4:3
    case "5/4":
      return "aspect-5/4";
    case "auto":
      ...
      >
        <div
          className={cn(
            ...
            selected ? "border-2 border-teal-500/50" : "border border-stone-500/10",
...
const ImageCarousel: React.FC<ImageCarousel_BasicProps> = ({
  ...
  if (images.length === 1) {
    return <ImageContainer aspectRatio={aspectRatio} classNameImage={classNameImage} image={images[0]} />;
  }
          ...
          <div className="-ml-4 flex">
            {images?.map((image, index) => (
              <div
                ...
                role="group"
              >
                <ImageContainer aspectRatio={aspectRatio} classNameImage={classNameImage} image={image} />
              </div>
            ))}
          </div>
        </div>
      ...
      {showThumbs && (thumbPosition === "bottom" || thumbPosition === "left" || thumbPosition === "right") && (
        ...
        >
          ...
          {showCarouselControls && (
            <>
              <Button
                className="-translate-y-1/2 -translate-x-1/2 absolute top-1/2 left-0 z-10 size-7"
              ...
              <Button
                className="-translate-y-1/2 absolute top-1/2 right-0 z-10 size-7 translate-x-1/2"
