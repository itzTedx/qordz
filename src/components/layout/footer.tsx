...
export const Footer = () => {
  return (
    <footer className="relative flex flex-col gap-y-12 overflow-hidden bg-foreground py-10 text-muted sm:gap-y-14 sm:py-12 md:gap-y-32 md:py-14 lg:py-16">
      <AnimatedGroup
        as="nav"
        className="container relative grid max-w-7xl grid-cols-2 gap-8 md:col-span-2 md:grid-cols-5 md:gap-4"
        variants={{
          container: {
            hidden: { opacity: 0 },
            visible: {
      ...
      >
        {FOOTER_LINKS.map((footer) => (
          <div className="space-y-3 md:space-y-5" key={footer.id}>
      ...
      <div className="container relative z-10 flex max-w-7xl items-center justify-between">
        <div>
          <Logo />
          <p className="text-center text-muted-foreground text-xs sm:text-sm">© 2025 Qordz. All Rights Reserved.</p>
        </div>
        <div className="flex flex-col items-end">
          <SocialChips />
          <p className="mt-1 text-stone-400 text-xs sm:text-sm md:text-base">Everyday comfort, everywhere.</p>
        </div>
      </div>
